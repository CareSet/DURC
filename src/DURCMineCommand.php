...
class DURCMineCommand extends Command{
	...
	foreach($db_struct as $db => $table_list){
		foreach($table_list as $this_table_name => $column_data){
			...
			if($last_character == 's'){
				echo "Fatal Error: $db->$this_table_name ends in an 's'. But Laravel tried to do smart things with plurals, which prevents autogenerated code from being easy. So for now, just rename your table so that it does not end in 's'\n";
				$fatal_error = true;
			}
