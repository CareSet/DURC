...
\t\t//You will have to resolve these recursive relationships in your code.";
    ...
    foreach ($fields as $create_mutator) {
        $mutator_method_name = "set".Str::studly($create_mutator['column_name'])."Attribute";
        $mutator_signature = "\tpublic function $mutator_method_name(\$value)\n";
        $field_name = $create_mutator['column_name'];
        $field_type = $create_mutator['data_type'];
        $mutator_body = "\t{\n";
        $mutator_body .= "\t\t\$formatted_value = DURC::formatForStorage('$field_name', '$field_type', \$value, \$this);\n";
        $mutator_body .= "\t\t\$this->attributes['$field_name'] = \$formatted_value;\n";
        $mutator_body .= "\t}\n\n";
        $mutators_code .= $mutator_signature.$mutator_body;
    }
